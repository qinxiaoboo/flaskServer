...
from flaskServer.services.internal.captcha.captcha import solve_geetest
from flaskServer.config.config import FAKE_TWITTER, HIDE_UNSUPPORTED, MAX_TRIES, REFERRAL_LINKS, SURVEYS, GALXE_CAMPAIGN_IDS
from flaskServer.services.internal.utils import wait_a_bit, get_query_param, get_proxy_url, async_retry, log_long_exc, plural_str

from flaskServer.services.internal.galxe.client import Client
from flaskServer.services.internal.galxe.fingerprint import fingerprints, captcha_retry
from flaskServer.services.internal.galxe.utils import random_string_for_entropy
from flaskServer.services.internal.galxe.models import Recurring, Credential, CredSource, ConditionRelation, QuizType, Gamification, GasType
from flaskServer.services.internal.galxe.constants import DISCORD_AUTH_URL, GALXE_DISCORD_CLIENT_ID, CHAIN_NAME_MAPPING, VERIFY_TRIES
...
class GalxeAccount:
    ...
    async def link_twitter(self, fake_twitter=FAKE_TWITTER):
        ...
        if existed_twitter_username == '':
            logger.warning(f"{self.idx}: 该环境没有绑定推特")
            # return
        if fake_twitter:
            return
        ...
        if existed_twitter_username != '':
            if existed_twitter_username.lower() == self.twitter.my_username:
                ...
                logger.warning(f'{self.idx}) Another Twitter account already linked with this EVM address: '
    ...
    async def link_email(self, strict=False):
        ...
        if existed_email != '':
            if existed_email.lower() == self.account.email_username.lower():
                return
            else:
                logger.warning(f"{self.idx}: 数据库邮箱用户和GALXE账号不匹配")
    ...
    async def link_discord(self):
        ...
        logger.info(f"{self.idx}: {existed_discord_username}")
        ...
        #         return
        #     else:
        #         logger.warning(f'{self.idx}) Another Discord account already linked with this EVM address: '
...
if __name__ == '__main__':
    ...
    from flaskServer.services.dto.env import getEnvByName
    from flaskServer.services.internal.utils.utils import wait_a_bit
    with app.app_context():
        account = getAccountByEnv(getEnvByName("Q-2"))
        galxe_account =  GalxeAccount(account.idx, account, account.evm_private_key)
        logger.info(f'{account.idx}) Galxe signing in')
        asyncio.run(galxe_account.login())
        logger.info(f'{account.idx}) Galxe signed in')
        asyncio.run( wait_a_bit())
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        for campaign_id in GALXE_CAMPAIGN_IDS:
            loop.run_until_complete(  galxe_account.complete_campaign(campaign_id) )
            loop.run_until_complete(  galxe_account.claim_campaign(campaign_id))

        asyncio.run(  wait_a_bit())

        logger.info(f'{account.idx}) Checking spaces stats')
        asyncio.run(  galxe_account.spaces_stats())