...
from flaskServer.config.config import MAX_TRIES, DISABLE_SSL

from flaskServer.services.internal.utils.async_web3 import AsyncHTTPProviderWithUA
...
def get_w3(useragent,rpc_url: str, proxy: str = None):
    proxy = get_proxy_url(proxy)
    req_kwargs = {} if proxy is None else {'proxy': proxy}
    if DISABLE_SSL:
        req_kwargs['ssl'] = False
    return AsyncWeb3(AsyncHTTPProviderWithUA(useragent,rpc_url, req_kwargs))

if __name__ == '__main__':
    w3 = get_w3("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36","https://rpc.ankr.com/eth","http://EJun1R:F4Zdth@45.93.213.122:8000")
    print(asyncio.run(w3.eth.max_priority_fee))